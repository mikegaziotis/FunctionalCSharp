...
{
    extension<T, TK, TE>(ValueTask<Result<Option<T>, TE>> resultTask)
    {
        /// <summary>
        ...
        /// operate on the innermost value, and is only called if it's present.
        /// </summary>
        public async ValueTask<Result<Option<TK>, TE>> BindOptional(Func<T, ValueTask<Result<TK, TE>>> bind)
        {
            var result = await resultTask.ConfigureAwait(DefaultConfigureAwait);
            ...
            return await result.Value.Match(
                some: BindSome,
                none: BindNone).ConfigureAwait(DefaultConfigureAwait);

            async ValueTask<Result<Option<TK>, TE>> BindSome(T v)
            {
                var bound = await bind(v).ConfigureAwait(DefaultConfigureAwait);
                var mapped = bound.Map(Option.From);
                return mapped;
            }

            ValueTask<Result<Option<TK>, TE>> BindNone()
                => Result.Success<Option<TK>, TE>(Option.None).AsCompletedValueTask();

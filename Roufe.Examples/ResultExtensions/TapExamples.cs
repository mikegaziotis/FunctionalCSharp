...
namespace Roufe.Examples.ResultExtensions
{
    public class TapExamples
    {
        ...
        Result<Unit,Error> DoWork() =>Unit.Value;
        Task<Result<Unit,Error>> DoWorkAsync() => Task.FromResult(Result.Success<Unit,Error>(Unit.Value));

        // A method that returns a Result<T>
        Result<Customer,Error> GetCustomer(int customer) => Result.Success<Customer,Error>(new Customer());
        Task<Result<Customer,Error>> GetCustomerAsync(int customer) => Task.FromResult(Result.Success<Customer,Error>(new Customer()));

...
public class BindTests_ValueTask_Left : TestBase
{
    [Fact]
    public async Task Bind_ValueTask_Left_returns_no_value_if_initial_Option_is_null()
    {
        Option<T> Option = null;

        var Option2 = await Option.AsValueTask().Bind(ExpectAndReturnOption<T>(null, T.Value2));
    ...
    public async Task Bind_ValueTask_Left_returns_no_value_if_selector_returns_null()
    {
        ...
        var Option2 = await Option.AsValueTask().Bind(ExpectAndReturn(T.Value, Option<T>.None));
    ...
    public async Task Bind_ValueTask_Left_returns_value_if_selector_returns_value()
    {
        ...
        var Option2 = await Option.AsValueTask().Bind(ExpectAndReturnOption<T>(T.Value, T.Value2));
    ...
    [Fact]
    public async Task Bind_ValueTask_Left_with_context_returns_no_value_if_initial_Option_is_null()
    {
        Option<T> Option = null;
